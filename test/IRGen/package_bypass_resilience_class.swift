...
package class Foo {
  ...
  // Core.Foo.deinit
  // CHECK-COMMON-DAG: define {{(dllexport |protected )?}}swiftcc ptr @"$s4Core3FooCfd"(ptr {{(nofree )?}}returned swiftself{{.*}} %0)
...
final package class Bar {
  ...
  // CHECK-COMMON-DAG: define {{(dllexport |protected )?}}swiftcc ptr @"$s4Core3BarCfd"(ptr {{(nofree )?}}returned swiftself{{.*}} %0)
