...
extension Function {
  ...
  /// Gets the global side effects of the function which are defined by attributes.
  ///
  ...
  /// The effect attributes are still dependent on the argument, because e.g. even readnone functions
  /// can read from indirect in-arguments and write to indirect out-arguments.
  public func getDefinedSideEffects(forArgument argumentIndex: Int? = nil) -> SideEffects.GlobalEffects {
    switch effectAttribute {
    case .none:
      return .worstEffects
    case .readNone:
      var result = SideEffects.GlobalEffects(ownership: .init(copy: true),
                                             traps: true)
      if let argIdx = argumentIndex {
        let convention = getArgumentConvention(for: argIdx)
        if convention.isIndirectIn {
          result.memory    = .init(read: true)
        } else if convention == .indirectOut {
          result.memory    = .init(write: true)
          result.ownership = .init()
        }
      }
      return result
    case .readOnly:
      var result = SideEffects.GlobalEffects(memory:    .init(read: true),
                                             ownership: .init(copy: true),
                                             traps: true)
      if let argIdx = argumentIndex, getArgumentConvention(for: argIdx) == .indirectOut {
        result.memory    = .init(write: true)
        result.ownership = .init()
      }
      return result
...
public struct SideEffects : CustomStringConvertible, NoReflectionChildren {
  ...
  fileprivate let arguments: [ArgumentEffects]
  ...
  var argumentEffectsCount: Int { arguments.count }
  ...
  /// ```
  public struct ArgumentEffects : CustomStringConvertible, NoReflectionChildren {
  ...
  public struct GlobalEffects : Equatable, CustomStringConvertible, NoReflectionChildren {
    ...
    public static var worstEffects: GlobalEffects {
      GlobalEffects(memory: .worstEffects, ownership: .worstEffects, traps: true, allocates: true)
    }
...
extension StringParser {
  ...
  mutating func parseEffectsFromSIL(to effects: inout FunctionEffects) throws {
    ...
    repeat {
      try parseEffectFromSIL(argumentIndex: argumentIndex, to: &effects)
    } while consume(",")
  }
  ...
  mutating func parseEffectFromSIL(argumentIndex: Int, to effects: inout FunctionEffects) throws {
    if consume("noescape") {
