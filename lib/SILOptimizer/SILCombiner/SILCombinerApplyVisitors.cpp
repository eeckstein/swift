...
SILCombiner::buildConcreteOpenedExistentialInfoFromSoleConformingType(
  ...
  if (auto *OER = dyn_cast<OpenExistentialRefInst>(OAI.OpenedArchetypeValue)) {
    ...
    if (OER->getOwnershipKind() == OwnershipKind::Owned &&
        !ArgOperand.isConsuming()) {
      // See buildConcreteOpenedExistentialInfo: neither casting `OER` directly
      // nor casting a borrow of it works when `ConcreteType` is itself an
      // opened archetype rather than a genuinely concrete type.
      if (isa<ExistentialArchetypeType>(ConcreteType))
        return std::nullopt;

      // Casting `OER` directly would consume it, leaving any other uses of
      // `OER` (e.g. a sibling `store_borrow` when CSE has merged multiple
      // opens of the same existential, or `OER`'s own `destroy_value`) without
      // a valid value to reference. We'd rather cast a borrow instead, so
      // `OER` itself stays alive and correctly typed for its other uses,
      // without paying for a retain/release -- but defer actually building
      // that borrow+cast until createApplyWithConcreteType confirms this
      // argument is rewritable. Building it eagerly here risks leaving a
      // `begin_borrow` with no matching `end_borrow` (and no use at all) if
      // the apply turns out not to be rewritable, e.g. because its result type
      // also refers to the opened archetype (a method returning `Self`).
      SoleCEI.ConcreteValueNeedsFixup = true;
    } else {
      // Either `OER` isn't owned, so casting it directly can't consume
      // anything it shouldn't; or the apply consumes this argument (@owned),
      // so casting `OER` directly at the apply site consumes it exactly as
      // the original, unspecialized apply would have — a borrow can't
      // satisfy an @owned parameter, and a borrow scope opened here wouldn't
      // have a destroy to close it against (see the sibling-fixup loop in
      // createApplyWithConcreteType).
      auto loc = RegularLocation::getAutoGeneratedLocation();
      SILBuilderWithScope b(ArgOperand.getUser(), Builder);
...
SILCombiner::buildConcreteOpenedExistentialInfo(Operand &ArgOperand) {
  ...
  if (COEI.CEI) {
    ...
    if (ConcreteValue->getFunction()->hasOwnership() &&
        COEI.OAI.OpenedArchetypeValue->getOwnershipKind() ==
            OwnershipKind::Owned) {
      // Make sure that the existential value outlives the apply. This is the case
      // if the apply uses it as argument operand.
      ...
      auto loc = RegularLocation::getAutoGeneratedLocation();
      if (!ArgOperand.isConsuming()) {
        // `ConcreteType` can itself be an opened archetype -- a
        // forwarding/recursive existential, e.g. an already-opened existential
        // re-wrapped into a less specific protocol -- rather than a genuinely
        // concrete type. Neither strategy below works in that case: the
        // deferred borrow+cast never reaches a fixed point because SILCombine
        // keeps re-simplifying the cast away, and casting
        // `OpenedArchetypeValue` directly would consume it even though this
        // argument doesn't, over-consuming it against its own `destroy_value`.
        // So don't propagate.
        if (isa<ExistentialArchetypeType>(COEI.CEI->ConcreteType))
          return std::nullopt;

        // We'd rather cast a borrow of `OpenedArchetypeValue` than
        // `OpenedArchetypeValue` itself, so that value stays alive and
        // correctly typed for any of its other uses -- including its own
        // `destroy_value` (it's owned, so casting it directly would consume
        // it) -- but defer actually building that borrow+cast until
        // createApplyWithConcreteType confirms this argument is rewritable.
        // `ConcreteValue` currently refers to the pre-existential value
        // already consumed by `init_existential`,
        // which cannot be reused directly; it's a placeholder that will be
        // overwritten once we know the borrow+cast will actually be used
        // (building it eagerly here risks leaving an unused `begin_borrow`
        // with no matching `end_borrow` if the apply turns out not to be
        // rewritable, e.g. because its result type also refers to the opened
        // archetype -- a method returning `Self`).
        // See `buildConcreteOpenedExistentialInfoFromSoleConformingType`.
        COEI.CEI->ConcreteValueNeedsFixup = true;
      } else {
        // The apply consumes this argument (@owned), so a borrow can't
        // satisfy that and a borrow scope opened here wouldn't have a destroy
        // to close it against. Cast `OpenedArchetypeValue` directly at the
        // apply site instead, consuming it exactly as the original
        // unspecialized apply would have.
        SILBuilderWithScope b(ArgOperand.getUser(), Builder);
...
SILInstruction *SILCombiner::createApplyWithConcreteType(
    FullApplySite Apply,
    llvm::SmallDenseMap<unsigned, ConcreteOpenedExistentialInfo> &COAIs,
    SILBuilderContext &BuilderCtx) {
  ...
  for (unsigned EndIdx = Apply.getNumArguments(); ArgIdx < EndIdx; ++ArgIdx) {
    ...
    ConcreteExistentialInfo &CEI = *ArgIt->second.CEI;
    assert(CEI.isValid());
    ...
    // We know this argument is rewritable. If buildConcreteOpenedExistentialInfo(*)
    // deferred building the borrowed cast (to avoid leaving an unused,
    // unclosed begin_borrow if this point was never reached), build it now.
    if (CEI.ConcreteValueNeedsFixup) {
      auto loc = RegularLocation::getAutoGeneratedLocation();
      SILBuilderWithScope b(OAI.OpenedArchetypeValue->getNextInstruction(),
                            BuilderCtx);
      SILValue castOperand = b.createBeginBorrow(loc, OAI.OpenedArchetypeValue);
      CEI.ConcreteValue = b.createUncheckedRefCast(
          loc, castOperand, Apply.getFunction()->getLoweredType(CEI.ConcreteType));
    }
  ...
  for (auto &valuePair : valuesToReplace) {
    ...
    // `uncheckedCast` is a forwarded view of the borrow scope opened on
    // `openedExistential` (see buildConcreteOpenedExistentialInfo*), so
    // `openedExistential` itself was never consumed by devirtualization and
    // remains live and correctly typed for uses that can't be redirected to
    // the concrete type below (e.g. `store_borrow`, which requires its
    // operand to exactly match a fixed destination type) — those are simply
    // left unchanged.
    auto *borrow = cast<BeginBorrowInst>(
        cast<UncheckedRefCastInst>(uncheckedCast)->getOperand());
    ...
    for (auto use : uses) {
      ...
      if (isa<DestroyValueInst>(user) || use->isConsuming()) {
        consumingUses.push_back(use);
        continue;
      }
      // Any other use (e.g. `store_borrow`) needs `openedExistential`'s
      // original type; it's still live and correctly typed, so leave it
      // unchanged.
    }
    // The borrow scope must close before `openedExistential` is destroyed;
    // end it right where `openedExistential`'s own destroys are, which by
    // construction already occur after every one of its uses.
    for (auto *use : consumingUses) {
      SILBuilderWithScope endBorrowBuilder(use->getUser(), BuilderCtx);
      endBorrowBuilder.createEndBorrow(
          RegularLocation::getAutoGeneratedLocation(), borrow);
    }
  }
